...
(defun compute-move (g cutoff-depth)
  (let*
    ((statty (make-stats))
     ...
     (num-potential-done (stats-num-potential-done statty))
          ...
          (progn
           ...
           (when
             ...
             (if (= curr-depth 0)
               ...
               ;; return nil
               (if (not (= curr-max-ind -1))
                 (return-from compute-max (svref paths current-max-ind))
...
(defun my-test ()
  (problem "2R + K vs. K")
  (let ((g (init-game (list (list *king* 1 0)
                            (list *rook* 4 0)
                            (list *rook* 5 2))
                      (list (list *king* 3 3)))))
    (compute-do-and-show-n-moves g 4 6)
